...
################################################################################
# ACADISTRA COMPLETE SYSTEM MIGRATION
# 
# This script performs a FULL database migration for the entire system.
# It will:
#   1. Backup existing database
#   2. Drop and recreate the database (FRESH START)
#   3. Rebuild all Docker containers with latest code
#   4. Run complete migration for ALL 65 tables
#   5. Seed initial data (admin user, standard subjects)
...
if docker ps | grep -q acadistra_postgres; then
    ...
    docker exec acadistra_postgres pg_dump -U acadistra acadistra > "$BACKUP_FILE" 2>/dev/null || {
        echo -e "${YELLOW}Warning: Could not backup database (might not exist yet)${NC}"
        touch "$BACKUP_FILE"
    }
...
docker volume rm schoolmanagementsystem_postgres_data 2>/dev/null || {
    echo -e "${YELLOW}No existing volume found${NC}"
}
echo -e "${GREEN}✓ Database volume removed (fresh start)${NC}"
